...
with desc_tab1:
    ...
    主導機能 (P1):その人の核となる価値観。息を吸うように自然に使える強み、またこだわる領域。\n
    創造機能 (P2):主機能の目的を達成する道具であり非常に器用。他者とのコミュニケーションや問題解決に積極的に活用される。\n
    役割機能 (P3):ある程度はこなせるが、長時間使うと疲弊する。自信はなく、あくまで社会に適応するために必要に迫られて使用。\n
    脆弱機能 (P4):意識しているが上手く扱えず、他人から強要されると動揺したり、拒絶反応を示すこともある。\n
    暗示機能 (P5):自分ではほとんど使えないが、求めているものであり、この要素を持っている人に強く惹かれる。\n
    動員機能 (P6):未発達ではあるが、自分で好んで使い、得意になりたいと努力する。\n
    無視機能 (P7):P1と同じくらい強い能力を持ちうまくこなせるが、価値を感じにくく、意図的に無視する。\n
    示範機能 (P8):非常に強い機能であると同時に、もっとも無自覚。
    
    価値機能 (Valued):P1, P2, P5, P6。これらを共有する人とは意気投合しやすいと言われています（クアドラ）。\n
    
    ---
    この診断では、能力ではなく、あなたがどの要素を大切にしたいか（価値）、また自覚的であるかどうかに基づき、モデルAの構造に当てはめて判定しています。
    """)
...
if st.button("診断結果を算出", type="primary", use_container_width=True):
    ...
    with col_b:
        ...
        st.success(f"**P6 (活性化): {funcs[5]}**")
        st.error(f"**P8 (示範): {funcs[7]}**")
    ...
    with col1:
        st.write("### 🥇 適合順位 (Top 3)")
        # スライス [:3] で上位3位に制限
        for i, (name, score) in enumerate(res['ranking'][:3]):
            st.write(f"{i+1}. **{name}**")
            st.caption(f"適合スコア: {score}点")
            
    with col2:
        st.write("### 心理機能の傾向")
        # グラフデータの作成
        df_scores = pd.DataFrame({
            "機能": list(res['scores'].keys()),
            "スコア": list(res['scores'].values())
        })
        # インデックスを「機能」にして、横棒グラフなどでマイナスを表示
        st.bar_chart(df_scores.set_index("機能"))

    st.info("※スコアがマイナスの機能は、あなたが価値を感じない（拒絶、または無関心）と回答した要素です。")
